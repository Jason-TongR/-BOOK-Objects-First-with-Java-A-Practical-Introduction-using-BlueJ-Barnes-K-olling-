...
public class MusicOrganizer
{
    ...
    public void listAllTracks()
    {
        ...
        trackList.stream().map(aTrack -> aTrack.getDetails())
    ...
    public void listWithIndex()
    {
        ...
        IntStream.range(0, trackList.size())
                 .mapToObj(index -> String.format("%3d: %s", index, trackList.get(index).getDetails()))
                 .forEach(details -> System.out.println(details));
        System.out.println();
    }
    
    /**
     * Find the index of the first track whose title contains the given search string.
    ...
    {
        OptionalInt first = IntStream.range(0, trackList.size())
               .filter(index -> trackList.get(index).getTitle().contains(searchString))
               .findFirst();
        return first.orElse(-1);
    }    
    
    /**
     * List all tracks by the given artist. The match may be partial.
     * @param artist The artist's name.
     */
    public void listByArtist(String artist)
    {
        trackList.stream().filter(aTrack -> aTrack.getArtist().contains(artist))
                          .map(aTrack -> aTrack.getDetails())
                          .forEach(details -> System.out.println(details));
    }
